...
public class TC10_P2_Placement_ValidateBlank_USAGE_YIELD_value extends WMS_TestBase {
	...
	public void setUp() throws InterruptedException {
		if (CloseBrowser) {
			...
			setReport("TC10_P2_Placement_ValidateBlank_USAGE_YIELD_value");
		}
	}

	@Test(priority = 0,dataProvider = "TC10_P2_Placement_ValidateBlank_USAGE_YIELD_value", dataProviderClass = DataProviders.class)
	public void P2_TC10_Placement_ValidateBlank_USAGE_YIELD_value(String TestType,String uploadFilepath,String resultFilepath,String filetype,String season
			,String colorway,String linesheetview) 
					throws Exception{
		if (CloseBrowser) {
			test = extent.createTest(":::TC10_P2_Placement_ValidateBlank_USAGE_YIELD_value:::");
		}
		CloseBrowser = false;
		...
		try {
			...
			test.log(Status.INFO, "This Testcase covers TC_184 to TC_185");
			...
			if(str[0].equalsIgnoreCase("Error")) {
				...
				customLoadersPage.Validate_USAGE_YIELD_Linesheet(colorway,test);
				test.log(Status.PASS, "Validation successful for USAGE/YIELD");
				System.out.println("Validation successful for USAGE/YIELD");
				addSrceenShot("Validation successful for USAGE/YIELD", test, Capture);
