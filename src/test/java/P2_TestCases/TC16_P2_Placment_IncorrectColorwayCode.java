...
public class TC16_P2_Placment_IncorrectColorwayCode extends WMS_TestBase {
	...
	public void setUp() throws InterruptedException {
		if (CloseBrowser) {
			...
			setReport("TC16_P2 Verify Error message for incorrect Colorway Code in placement file");
		}
	}

	@Test(priority = 0,dataProvider = "TC16_P2_Placment_IncorrectColorwayCode", dataProviderClass = DataProviders.class)
	public void P0_TC_02_Placement_BlankSeasonCode(String TestType,String uploadFilepath,String resultFilepath,String filetype) 
					throws Exception{
		if (CloseBrowser) {
			test = extent.createTest(":::TC16_P2 Verify Error message for incorrect Colorway Code in placement file:::");
		}
		CloseBrowser = false;
		...
		try {
			...
			test.log(Status.INFO, "This Testcase covers TC_186 to TC_187");
			...
			if(str[0].equalsIgnoreCase("Error")) {
				...
				if(str[1].equals("PC9 Code does not exist")) {
					test.log(Status.PASS, "Correct Error message is populating in sheet");
				}
				if(str[1].equals("PC9 Code does not exist in provided Season")) {
					test.log(Status.PASS, "Correct Error message is populating in sheet");
				}
			}
