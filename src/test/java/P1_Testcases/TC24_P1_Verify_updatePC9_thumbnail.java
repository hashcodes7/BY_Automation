...
public class TC24_P1_Verify_updatePC9_thumbnail extends WMS_TestBase {
	WebDriver driver;
	...
	public void setUp() throws InterruptedException {
		if (CloseBrowser) {
			...
			setReport("TC24_P1 Verify PC9 thumbnail");
		}
	}

	@Test(priority = 0,dataProvider = "TC24_P1_Verify_updatePC9_thumbnail", dataProviderClass = DataProviders.class)
	public void P1_TC_Product_024_VerifyPC9Thumbnail(String TestType,String season,String colorway,String colormenu, 
			String colorsubmenu ,String filtercolor,String productsegLSUSvalue,String productsegLSEvalue,String linesheetview,String hubofferedto) 
					throws Exception{
		if (CloseBrowser) {
			test = extent.createTest(":::TC24_P1 Verify PC9 thumbnail:::");
		}
		CloseBrowser = false;
		...
		try {
			test.log(Status.INFO, "This testcase covers TC_34");
...
//          if colorsubmenu element is not there just keep blank string ----------------------			
			...
			System.out.println("Thumbnail is matching ");
			test.log(Status.PASS, "Thumbnail is matching");
			addSrceenShot("Thumbnail is matching", test, Capture);
			
			Colorwaypage.Validate_BlankSeasonalAttributes_value(test);
			System.out.println("validation successful for seasonal Attributes");
			test.log(Status.PASS, "validation successful for seasonal Attributes");
			addSrceenShot("validation successful  for seasonal Attributes", test, Capture);
