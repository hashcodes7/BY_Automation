...
public class TC03_P0_Placements_Results_Blank_Brand extends WMS_TestBase {
	WebDriver driver;
	...
	public void setUp() throws InterruptedException {
		if (CloseBrowser) {
			...
			setReport("TC03_P0 Verify Error message for blank BRAND in placement file");
		}
	}

	@Test(priority = 0,dataProvider = "TC_03_P0_Placements_Results_Blank_Brand", dataProviderClass = DataProviders.class)
	public void TC_03_P0_Placements_Results_BlankBrand(String TestType,String uploadFilepath,String resultFilepath,String filetype) 
					throws Exception{
		if (CloseBrowser) {
			test = extent.createTest(":::TC03_P0 Verify Error message for blank BRAND in placement file:::");
		}
		CloseBrowser = false;
		...
		try {
			...
			test.log(Status.INFO, "This Testcase covers TC_191");
			...
			if(str[0].equalsIgnoreCase("Error")) {
				...
				if(str[1].equals("Brand is not provided in the load file")) {
					test.log(Status.PASS, "Correct Error message is populating in sheet");
					System.out.println("Correct Error message is populating in sheet");
				}
			}
